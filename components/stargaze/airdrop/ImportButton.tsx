  ...
  useEffect(() => {
    const validateList = (airdropList: Recipient[], airdropType: string) => {
      if (airdropList.length === 0) {
        setState({
          alertMsg: `Your airdrop list does not include any entries.`,
          alertSeverity: "error",
        });
        return false;
      }
      const indexOfEmptyAddress = airdropList.findIndex(
        (recipient: Recipient) =>
          recipient.address === undefined || recipient.address.length === 0
      );
      ...
      if (airdropType === "mint_for") {
        const indexOfEmptyToken = airdropList.findIndex(
          (recipient: Recipient) => recipient.token_id === undefined
        );
    ...
    const getUniqueAddresses = (airdropList: Recipient[]) => {
      ...
      airdropList.forEach((item: Recipient) => {
        const i = unique.findIndex((x) => x === item.address);
        if (i <= -1) {
    ...
    const handleCSV = (csvData: string) => {
      try {
        ...
        Papa.parse(csvData, {
          complete: function (results: any) {
            ...
            if (indexOfAddress === -1) {
              setState({
                alertMsg:
                  "Unable to find the 'address' column. Please make sure you have the same structure as the template file.",
                alertSeverity: "error",
              });
              return;
            }
        ...
        setState({
          alertMsg: `Format mismatch. Please confirm that your airdrop file has the same structure as the template.`,
          alertSeverity: "error",
        });
        console.error(e);
      }
    ...
    <Menu as="div" className="relative inline-block text-left z-20">
      ...
      >
        <Menu.Items className="origin-top-right absolute right-0 mt-1 w-full shadow-lg bg-white ring-1 border border-black ring-black ring-opacity-5 divide-y divide-gray-100 focus:outline-none">
          ...
          {/*<div className="py-1">*/}
          {/*    <Menu.Item>*/}
          {/*        <a className='group flex items-center px-4 py-2 text-sm  hover:text-lightgreen hover:cursor-pointer'  onClick={() => openFileSelector()}>*/}
          {/*            JSON*/}
          {/*        </a>*/}
          {/*    </Menu.Item>*/}
          {/*</div>*/}
