...
const InventoryToggle: FunctionComponent<StateProps> = ({state, setState}) => {
    if(state.airdropType === "mint_for") {
        return (
            <div className="flex justify-center items-center ml-4">
                <label htmlFor="inventory-toggle" className="inline-flex relative items-center cursor-pointer">
                    <input type="checkbox" value="" onChange={(e) => setState({dropFromInv: e.target.checked})}
                           id="inventory-toggle" className="sr-only peer" checked={state.dropFromInv} disabled={state.currentStep > 2}
                    />
                    <div
                    ...
                    <span
                        className="ml-3 text-sm md:text-base font-bold text-[#686765]">Drop from {state.dropFromInv ? "inventory" : "contract"}</span>
                </label>
            </div>
        );
    }
...
const NetworkToggle: FunctionComponent<StateProps> = ({state, setState}) => {
    return (
        <div className="flex justify-center items-center">
            <label htmlFor="network-toggle" className="inline-flex relative items-center cursor-pointer">
                <input type="checkbox" value="" onChange={(e) => setState({mainnet: e.target.checked})}
                       id="network-toggle" className="sr-only peer" checked={state.mainnet} disabled={state.currentStep > 2}
                />
                <div
                    className="w-11 h-6 bg-gray-200 dark:bg-gray-700 peer-checked:after:translate-x-full after:w-5
                            peer-checked:after:border-white after:absolute after:top-0.5 after:left-[2px] after:bg-white after:h-5
