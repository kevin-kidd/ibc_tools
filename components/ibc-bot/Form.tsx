...
const Form: FunctionComponent<StateProps> = (props) => {
  ...
  const displaySuccess = () => {
    setState({
      loading: false,
      alertMsg: "Your Discord has been successfully linked!",
  ...
  const linkDiscord = async () => {
    try {
      ...
      if (client === undefined) {
        displayError(
          "Unable to connect Keplr! Please make sure you have installed and logged in to your Keplr wallet."
        );
        return;
      }
      ...
      const execResponse = await setWhitelist(
        client,
        state.currentCollection.contract
      );
      ...
      const postResponse = await postData(
        ...
        state.discord,
        client.address,
        state.currentCollection.contract.address
      );
      ...
      if (e instanceof Error && e.message === "Request rejected") {
        ...
        displayError(
          "Unexpected error occurred. Please contact support for help."
        );
      }
      console.error(e);
    }
  ...
  return (
    <div>
      ...
      <div className="justify-center content-center flex mb-10">
        <p>
          <a
            href="https://patrik768772.typeform.com/IBCBOT"
            target="_blank"
            rel="noreferrer"
            className="hover:text-lightgreen font-bold"
          >
            Add IBC Bot
          </a>{" "}
