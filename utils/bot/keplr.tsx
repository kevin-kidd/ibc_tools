...
import type { Contract } from "../../types/bot";
...
export const setWhitelist = async (
  ...
  const response = await client.tx.broadcast([whitelistTx], {
    gasLimit: 150_000,
  });
  if (response.code === 0) {
    return response;
...
const getAddress = async () => {
  if (!window.getOfflineSignerOnlyAmino) return;
      ...
      msgs: [
        {
          type: "link-discord",
          value:
            "This is a signature request. Accepting this request will allow us to verify ownership of your NFTs. " +
